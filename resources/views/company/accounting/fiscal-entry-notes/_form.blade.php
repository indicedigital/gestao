...
<div class="row">
    <div class="col-md-6 mb-3">
        <label for="client_id" class="form-label">Cliente cadastrado (opcional)</label>
        <select class="form-select @error('client_id') is-invalid @enderror" id="client_id" name="client_id">
            <option value="">— Preencher manualmente —</option>
            @foreach($clients as $c)
                <option value="{{ $c->id }}" @selected(old('client_id', $note->client_id ?? null) == $c->id)>{{ $c->name }}</option>
            @endforeach
        </select>
        <small class="text-muted">Selecionar preenche os dados do tomador para emissão da NF.</small>
        @error('client_id')
        <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
    <div class="col-md-3 mb-3">
        <label for="person_type" class="form-label">Tipo <span class="text-danger">*</span></label>
...
<div class="row">
    <div class="col-md-4 mb-3">
        <label for="amount_received" class="form-label">Valor recebido (R$) <span class="text-danger">*</span></label>
    ...
    <div class="col-md-4 mb-3">
        ...
        <input type="date" class="form-control @error('received_date') is-invalid @enderror" id="received_date" name="received_date" value="{{ old('received_date', isset($note) && $note->received_date ? $note->received_date->format('Y-m-d') : now()->format('Y-m-d')) }}" required>
        @error('received_date')
        <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
...
(function () {
    ...
    const byId = Object.fromEntries(clients.map(c => [String(c.id), c]));
    const sel = document.getElementById('client_id');
    if (!sel) return;
    const fields = ['person_type','client_name','client_email','client_phone','document','document_type','address','city','state','zip_code','country'];
