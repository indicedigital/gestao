...
<div class="container-fluid py-4">
    ...
    <div class="page-header-modern mb-4">
        <div class="d-flex justify-content-between align-items-center flex-wrap">
            <div>
                ...
                <p>Atualize os dados da despesa</p>
            </div>
            <a href="{{ route('company.expenses.index', ['type' => $expense->type]) }}" class="btn btn-secondary">
    ...
    <div class="card-modern">
        <div class="card-body">
            <form action="{{ route('company.expenses.update', $expense) }}" method="POST" id="expenseForm">
                @csrf
                @method('PUT')
                ...
                <div class="row mb-4">
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required onchange="toggleExpenseFields()">
                            <option value="fixed" {{ old('type', $expense->type) === 'fixed' ? 'selected' : '' }}>Fixa (Recorrente mensal)</option>
                            <option value="variable" {{ old('type', $expense->type) === 'variable' ? 'selected' : '' }}>Variável (Única)</option>
                        </select>
                        @error('type')
                ...
                <div class="row">
                    <div class="col-md-4 mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" step="0.01" class="form-control @error('value') is-invalid @enderror" id="value" name="value" value="{{ old('value', $expense->value) }}" required min="0">
                        </div>
                        @error('value')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="row" id="fixed-fields" style="display: {{ old('type', $expense->type) === 'fixed' ? 'flex' : 'none' }};">
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('due_date_day') is-invalid @enderror" id="due_date_day" name="due_date_day">
                            ...
                            @for($i = 1; $i <= 31; $i++)
                            <option value="{{ $i }}" {{ old('due_date_day', $expense->due_date_day) == $i ? 'selected' : '' }}>Dia {{ $i }}</option>
                            @endfor
                        </select>
                        @error('due_date_day')
                        <div class="invalid-feedback">{{ $message }}</div>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <select class="form-select @error('is_active') is-invalid @enderror" id="is_active" name="is_active">
                            <option value="1" {{ old('is_active', $expense->is_active) ? 'selected' : '' }}>Ativa</option>
                            <option value="0" {{ !old('is_active', $expense->is_active) ? 'selected' : '' }}>Inativa</option>
                        </select>
                        <small class="text-muted">Despesas inativas não geram novas duplicatas</small>
                        @error('is_active')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row" id="variable-fields" style="display: {{ old('type', $expense->type) === 'variable' ? 'flex' : 'none' }};">
                    <div class="col-md-6 mb-3">
                        <label for="due_date" class="form-label">Data de Vencimento <span class="text-danger">*</span></label>
                        <input type="date" class="form-control @error('due_date') is-invalid @enderror" id="due_date" name="due_date" value="{{ old('due_date', $expense->due_date ? $expense->due_date->format('Y-m-d') : '') }}">
                        @error('due_date')
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="d-flex justify-content-end gap-2 mt-4">
                    <a href="{{ route('company.expenses.index', ['type' => $expense->type]) }}" class="btn btn-secondary">Cancelar</a>
